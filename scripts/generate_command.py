...
def parse_frame(data: bytes) -> dict[str, bytes | int]:
    """Parse a protocol frame and verify the CRCs."""
    if len(data) < 18 or data[:2] != b"\xAA\x02":
        raise ValueError("invalid header")
    size = int.from_bytes(data[2:4], "little")
    if _crc8(data[:4]) != data[4:5]:
        raise ValueError("CRC8 mismatch")
    if _crc16(data[:16 + size]) != data[16 + size:18 + size]:
...
_CMD_NAMES = {
    (2, 1, 5): "get_product_info",
    (2, 1, 64): "get_cpu_id",
    (2, 1, 65): "get_serial_main",
    (6, 1, 65): "get_serial_extra",
    (2, 32, 2): "get_pd",
    (3, 32, 2): "get_ems_main",
    (4, 32, 2): "get_inverter",
    (6, 32, 2): "get_ems_extra",
    (2, 32, 40): "get_lcd",
    (4, 32, 66): "ac_switch",
    (4, 32, 72): "get_dc_in_current",
    (4, 32, 74): "get_fan_auto",
    (4, 32, 84): "get_lab",
}
